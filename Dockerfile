...
RUN mv /usr/forhexo-2/public/* $NGINX_HTML_DIR/app
# 给nginx.conf配置文件授权
RUN chmod +rw /usr/forhexo-2/nginx-server.conf
# 覆盖原nginx.conf文件
RUN mv /usr/forhexo-2/nginx-server.conf $NGINX_INCLUED_CONFIG_DIR/default.conf
# 收尾工作，删除不必要的文件
#RUN rm -rf /usr/hexo-blog
